...
const showResult2D = (title, containerId, dataArray) => {
    ...
    for (let i = 0; i < dataArray.length; i++) {
        ...
        for (let j = 0; j < dataArray[i].length; j++) {
            let td = document.createElement('td');
            let span = document.createElement('span');
            span.textContent = dataArray[i][j]; // Set the text content of the span to the array value
            td.appendChild(span);
            tr.appendChild(td);
        }
        table.appendChild(tr);
...
function performOperation(operation) {
    ...
    // Call the appropriate matrix calculation functions based on the operation
    if (operation === 'add') {
...
function addMatrices(matrix1, matrix2) {
    if (matrix1.length !== matrix2.length || matrix1[0].length !== matrix2[0].length) {
        console.error('Matrix addition is not possible: Matrices have different dimensions.');
        return null;
    }
...
const subtractMatrices = function (matrix1, matrix2) {
    if (matrix1.length !== matrix2.length || matrix1[0].length !== matrix2[0].length) {
        console.error('Matrix subtraction is not possible: Matrices have different dimensions.');
        return null;
    }
...
const multiplyMatrices = (matrix1, matrix2) => {
    if (matrix1[0].length !== matrix2.length) {
        console.error('Matrix multiplication is not possible: Incompatible dimensions.');
        return null;
    }
